...
int main() {
  ...
  do {
    for (i = 0; i < 10; i++) {
      cout << i + 1 << ". " << n[i].get_name() << " Price: " << n[i].get_price()
           << " Amount Left: " << n[i].get_amount() << endl;
    }
    ...
    do {
      ...
      if (c > n[m - 1].get_amount()) {
        cout << "Cannot complete the request due to amount exceeding the stock."
             << endl;
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (n[i].return_check() == true) {
      l.insert(n[i].return_bought(),n[i].get_name());
      //s[x] = new pair<string, int>(n[i].get_name(), n[i].return_bought());
      x++;
    }
  }
  //sorting(s, x);
